...
Your task is to build a client-server system to represent an ASCII art shape canvas. The exercise involves two pieces:

- A server that will implement the shape endpoints into a canvas.
- A _read-only_ client that will show the canvas stored by the server with the shapes.
...
- simple: you can implement the shape operations with constant-width characters.
- runnable with clear running instructions.
