defmodule Canvas.Factory.EctoReturningStrategy do
  @moduledoc """
  Strategy with returning: true

  https://github.com/thoughtbot/ex_machina/issues/273
  https://github.com/thoughtbot/ex_machina/issues/280
  """
  use ExMachina.Strategy, function_name: :create

  def handle_create(%{__meta__: %{state: :loaded}} = record, _) do
    raise "You called `create` on a record that has already been inserted.
  ...
  def handle_create(%{__meta__: %{__struct__: Ecto.Schema.Metadata}} = record, %{repo: repo}) do
    record
    |> cast
    |> repo.insert!(returning: true)
  end

  def handle_create(record, %{repo: _repo}) do
    raise ArgumentError, "#{inspect(record)} is not an Ecto model. Use `build` instead"
  end

  def handle_create(_record, _opts) do
